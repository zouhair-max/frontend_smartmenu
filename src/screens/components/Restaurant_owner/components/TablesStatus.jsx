  ...
  return (
    <div className="bg-white rounded-lg shadow">
      <div className="p-4 sm:p-6 border-b border-gray-200">
        <h2 className="text-base sm:text-lg font-semibold text-gray-900">Tables Status</h2>
      ...
      <div className="p-3 sm:p-4 border-b border-gray-200">
        <div className="grid grid-cols-2 gap-2">
          {Object.entries(statusCounts).map(([status, count]) => {
            ...
            return (
              <div key={status} className="flex items-center justify-between p-1.5 sm:p-2 bg-gray-50 rounded">
                <span className={`inline-flex items-center px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-xs font-medium ${statusInfo.color}`}>
                  {statusInfo.label}
                </span>
                <span className="font-semibold text-sm sm:text-base text-gray-900">{count}</span>
              </div>
            );
          })}
        </div>
        ...
        {tables.length === 0 ? (
          ...
          <div className="divide-y divide-gray-200">
            {tables.map((table) => {
              ...
              return (
                <div key={table.id} className="p-3 sm:p-4 hover:bg-gray-50">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-sm sm:text-base text-gray-900">{table.name}</p>
